...
import { Upload, Table, Card, Button, Icon, Popover, Input, InputNumber, Modal, Select, Row, Col, Badge } from 'antd';
...
import { init, storeAlgorithm, applyAlgorithm, deleteAlgorithmFromStore } from '../../../actions/algorithmTable.action';
      ...
      },
      {
        ...
        render: (text, record, index) => (
          <Row type="flex" justify="start">
            <Col span={4} >
              <Button type="edit" shape="circle" icon="edit"
                onClick={() => {
      ...
      <div style={{ height: 'auto', width: '400px' }}>
        <Row style={{ marginBottom: 5 }}>
          <Input
            ...
            value={algoData.name}
            onChange={(e) => this.setState({ algoToAdd: { ...this.state.algoToAdd, name: e.target.value } })}
        ...
        <Row style={{ marginBottom: 5 }}>
          <Input
            ...
            onChange={(e) => this.setState({ algoToAdd: { ...this.state.algoToAdd, algorithmImage: e.target.value } })}
        ...
        <Row style={{ marginBottom: 5 }}>
          ...
          <InputNumber
            ...
            onChange={(v) => this.setState({ algoToAdd: { ...this.state.algoToAdd, mem: v + this._parseUnit(algoData.mem).unit } })}
            style={{ width: 'auto' }} />
          <Select
            value={memory.unit}
            defaultValue={memory.unit}
            style={{ width: '90px' }}
            onChange={(v) => this.setState({ algoToAdd: { ...this.state.algoToAdd, mem: this._parseUnit(algoData.mem).val + v } })}>
...
export default connect(mapStateToProps, { openModal, init, storeAlgorithm, applyAlgorithm, deleteAlgorithmFromStore })(
