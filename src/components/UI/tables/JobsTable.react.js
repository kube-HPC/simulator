...
class JobsTable extends Component {
  componentWillMount() {
    ...
    const getStatusFilter = () => {
      return Object.keys(PIPELINE_STATUS).map(k => ({
        text: firstLetterUpperCase(k),
        value: k
      }));
    };
        ...
        render: (text, record) => {
          return (
            <span>
              {record.results ? (
                <span>
                  {humanizeDuration(record.results.timeTook * 1000, {
                    maxDecimalPoints: 2
                  })}
                </span>
              ) : (
              ...
              ? Object.entries(record.status.data.states.asMutable()).map(
                  (s, i) => (
                    <Tooltip
                      key={s[0]}
                      placement="top"
                      title={firstLetterUpperCase(s[0])}
                    >
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <Tooltip
              placement="top"
              title={PRIORITY[record.pipeline.priority].name}
            >
          ...
          const downloadAction = (
            <Tooltip placement="top" title={'Download Results'}>
              <Button
                ...
                icon="download"
                onClick={() =>
                  this.downloadStorageResults(
                    record.results.data.storageInfo.path
                  )
                }
              />
            </Tooltip>
