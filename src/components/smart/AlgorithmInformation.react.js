    ...
    return (
      <Tabs defaultActiveKey="1">
        <Tabs.TabPane tab="Logs" key="1">
          <Logs
            log={this.props.logs.dataSource.asMutable()}
            taskDetails={
              data && data.payload
                ? data.payload.batch && data.payload.batch.length
                  ? data.payload.batch
                  : [{ taskId: data.payload.taskId }]
                : null
            }
            rerunLogs={(taskId = data.payload.taskId) =>
              data && this.props.getKubernetesLogsData(taskId)
            }
          />
        </Tabs.TabPane>
        ...
        <Tabs.TabPane tab="Caching" key="3">
          <Caching
            runCaching={() =>
              data && this.props.getCaching(data.payload.jobId, data.payload.nodeName)
            }
          />
        </Tabs.TabPane>
...
export default connect(
  mapStateToProps,
  { sideBarOpen, sideBarClose, getKubernetesLogsData, getCaching }
