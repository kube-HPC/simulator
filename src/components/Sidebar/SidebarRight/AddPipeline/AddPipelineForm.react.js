...
import DynamicForm from 'components/Sidebar/SidebarRight/AddPipeline/AddPipelineFormDynamic.react';
...
export default function AddPipelineForm(props) {
  ...
  if (!webhooks) formData.webhooks = { progress: '', result: '' };
    ...
    webhooks.progress = webhooks.progress ? webhooks.progress.replace(URL_REGEX, '') : '';
    webhooks.result = webhooks.result ? webhooks.result.replace(URL_REGEX, '') : '';
  }
        ...
        <Input
          addonBefore={
            <SelectBefore
              ...
              setValue={progress => setSelectedWebhook(prev => ({ ...prev, progress }))}
            />
          }
          placeholder="Progress Webhook URI"
          onChange={e => {
            formData.webhooks.progress = `${selectedWebhook.progress}${e.target.value}`;
        ...
        <Input
          addonBefore={
            <SelectBefore
              ...
              setValue={result => setSelectedWebhook(prev => ({ ...prev, result }))}
            />
          }
          placeholder="Result Webhook URI"
          onChange={e => {
            formData.webhooks.result = `${selectedWebhook.result}${e.target.value}`;
            setFormData({ ...formData });
          }}
        ...
        <Row type="flex" gutter={10}>
          ...
          <Col span={6}>
            <InputNumber
              ...
              max={100}
              value={formData.options.batchTolerance}
              onChange={e => !isNaN(e) && onChangeTarget(formData, 'options', 'batchTolerance')(e)}
            />
          </Col>
        </Row>
      </Form.Item>
        ...
        <Row type="flex" gutter={10}>
          ...
          <Col span={6}>
            <InputNumber
              ...
              value={formData.options.concurrentPipelines}
              onChange={e =>
                !isNaN(e) && onChangeTarget(formData, 'options', 'concurrentPipelines')(e)
              }
            />
          </Col>
        </Row>
        ...
        <Row>
          <Col>
            <InputNumber
              ...
              onChange={e => !isNaN(e) && onChangeTarget(formData, 'options', 'ttl')(e)}
            />
          </Col>
          <Col />
        </Row>
  ...
  const [editorValue, setEditorValue] = useState(stringify(addPipelineTemplate));
  return (
    <div>
      {editorIsVisible && (
      ...
      <BottomContent
        extra={[
          ...
          <Popover key="clear" content={<div>Resets {editorIsVisible ? 'JSON' : 'Wizard'}</div>}>
            <Button
              type="danger"
              onClick={() => {
                if (editorIsVisible) setEditorValue(stringify(addPipelineTemplate));
                else {
                  setFormData({
                    ...addPipelineTemplate,
                    nodes: [
